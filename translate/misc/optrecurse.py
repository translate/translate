...
class RecursiveOptionParser(argparse.ArgumentParser, object):
    ...
    def format_manpage(self):
        ...
        formatToolkit = "Translate Toolkit %s" % __version__.sver
        result.append('.\\" Autogenerated manpage\n')
        result.append('.TH %s 1 "%s" "" "%s"\n' % (
            prog, formatToolkit, formatToolkit))
        ...
        result.append('.PP\n')
        usage = "\\fB%prog "
        usage += " ".join([self.getusageman(option) for option in self._get_optional_actions()])
        usage += "\\fP"
        result.append('%s\n' % formatprog(usage))
    ...
    def getusageman(self, option):
        """returns the usage string for the given option"""
        optionstring = "\\fR|\\fP".join(option.option_strings)
        if getattr(option, "optionalswitch", False):
        ...
        if self.isrecursive(options.input, 'input') and getattr(options, "allowrecursiveinput", True):
            if not self.isrecursive(options.output, 'output'):
                if not options.output:
                    self.error(argparse.ArgumentError("No output directory given"))
                try:
