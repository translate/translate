...
class RecursiveOptionParser(optparse.OptionParser, object):
  ...
  def format_manpage(self):
    ...
    result = []
    prog = optparse.get_prog_name()
    formatprog = lambda x: x.replace("%prog", prog)
    result.append('.\" Autogenerated manpage\n')
    result.append('.TH %s\n' % formatprog(self.version))
    result.append('.SH NAME\n')
    result.append('%s - %s\n' % (optparse.get_prog_name(), self.description))
    result.append('.SH SYNOPSIS\n')
    result.append('.PP\n')
    result.append('%s\n' % formatprog(self.usage))
    result.append('.SH OPTIONS\n')
    self.formatter.store_option_strings(self)
    result.append('.PP\n')
    for option in self.option_list:
      result.append(self.formatter.format_option(option)+"\n")
    return "".join(result)
