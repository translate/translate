...
class ProjectManager(object):
    """Manages the processes involved in a project workflow."""
    ...
    def add_source(self, srcfile, src_fname=None):
        return self.project.append_sourcefile(srcfile, src_fname)
    ...
    def export_file(self, fname, destfname):
        """Export the file with the specified filename to the given destination.
            ...
            L{Project.get_file()} if C{fname} is not found in the project."""
        open(destfname, 'w').write(self.project.get_file(fname).read())

    def get_file(self, fname):
        """Proxy for C{Project.get_file()}."""
        return self.project.get_file(fname)

    def get_proj_filename(self, realfname):
        """Try and find a project file name for the given real file name."""
        for fname in self.project._files:
            if fname == realfname or self.project._files[fname] == realfname:
                return fname
        raise ValueError('Real file not in project: %s' % (realfname))
    ...
    def convert_forward(self, input_fname, template=None, output_fname=None, append_output_ext=True, convert_options=None):
        ...
        input_type = self.project.get_filename_type(input_fname)
        ...
        output_type = self.project.TYPE_INFO['next_type'][input_type]
        outputfile, output_fname = self.project.append_file(output_fname, None, ftype=output_type)
