...
class Record(UserDict):
    def __init__(
        self, record_keys, record_values=None, compute_derived_values=lambda x: x
    ):
...
class FileTotals:
    ...
    def _compute_derived_values(cls, record):
        record["total"] = (
            record["untranslated"] + record["translated"] + record["fuzzy"]
        )
        record["totalsourcewords"] = (
            record["untranslatedsourcewords"]
...
class StatsCache:
    ...
    def __new__(cls, statsfile=None):
        ...
        def make_database(statsfile):
            ...
            cache = cls._caches.setdefault(current_thread_ident, {})[
                statsfile
            ] = object.__new__(cls)
            connect(cache)
        ...
        if (
            current_thread_ident in cls._caches
            and statsfile in cls._caches[current_thread_ident]
        ):
    ...
    def create(self):
        ...
        self.cur.execute(
            """CREATE UNIQUE INDEX IF NOT EXISTS filepathindex
            ON files (path);"""
        )
        ...
        self.cur.execute(
            """CREATE INDEX IF NOT EXISTS fileidindex
            ON units(fileid);"""
        )

        self.cur.execute(
            """CREATE TABLE IF NOT EXISTS checkerconfigs(
            configid INTEGER PRIMARY KEY AUTOINCREMENT,
            config VARCHAR);"""
        )

        self.cur.execute(
            """CREATE INDEX IF NOT EXISTS configindex
            ON checkerconfigs(config);"""
        )

        self.cur.execute(
            """CREATE TABLE IF NOT EXISTS uniterrors(
        ...
        self.cur.execute(
            """CREATE INDEX IF NOT EXISTS uniterrorindex
    ...
    def _getstoredcheckerconfig(self, checker):
        ...
        self.cur.execute(
            """SELECT configid, config FROM checkerconfigs WHERE
            config=?;""",
            (config,),
        )
    ...
    @transaction
    def _cacheunitstats(
        self, units, fileid, unitindex=None, file_totals_record=FileTotals.new_record()
    ):
        ...
        for index, unit in enumerate(units):
            if unit.istranslatable():
                ...
                file_totals_record = file_totals_record + FileTotals.new_record(
                    unit_state_for_db, sourcewords, targetwords
                )
    ...
    def file_extended_totals(self, filename, store=None):
        ...
        self.cur.execute(
            """SELECT e_state, COUNT(id), SUM(sourcewords), SUM(targetwords)
            FROM units WHERE fileid=? GROUP BY e_state""",
            (fileid,),
        )
        ...
        for index, unit in enumerate(units):
            if unit.istranslatable():
                ...
                for checkname, checkmessage in failures.items():
                    unitvalues.append(
                        (index, fileid, configid, checkname, checkmessage)
                    )
    ...
    def get_unit_stats(self, fileid, unitid):
        ...
        if result is not None:
            ...
            logger.warning(
                "Database in inconsistent state - fileid %d and "
    ...
    def recacheunit(self, filename, checker, unit):
        ...
        unitid = unit.getid()
        # get the unit index
        totals_without_unit = self.file_totals[fileid] - FileTotals.new_record(
            *self.get_unit_stats(fileid, unitid)
        )
        self.cur.execute(
            """SELECT unitindex FROM units WHERE
            fileid=? AND unitid=?;""",
            (fileid, unitid),
        )
        ...
        self.cur.execute(
            """DELETE FROM uniterrors WHERE
            fileid=? AND unitindex=?;""",
            (fileid, unitindex),
        )
        if os.path.exists(suggestion_filename(filename)):
            checker.setsuggestionstore(
                factory.getobject(
                    suggestion_filename(filename), ignore=suggestion_extension()
                )
            )
        state.extend(
            self._cacheunitschecks([unit], fileid, configid, checker, unitindex)
        )
        return state
    ...
    def _get_config_id(self, fileid, checker):
        ...
        if configid:
            return configid
        self.cur.execute(
            "INSERT INTO checkerconfigs " "(configid, config) values (NULL, ?);",
    ...
    def file_fails_test(self, filename, checker, name):
        ...
        self.cur.execute(
            ...
            unitindex
            FROM uniterrors
            WHERE fileid=? and configid=? and name=?;""",
            (fileid, configid, name),
        )
    ...
    def filestatestats(self, filename, store=None, extended=False):
        ...
        self.cur.execute(
            """SELECT state, e_state, unitindex
            FROM units WHERE fileid=? ORDER BY unitindex;""",
            (fileid,),
        )
        values = self.cur.fetchall()
