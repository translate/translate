---
title: Translate Toolkit 2.0.0b1 released
category: releases
---
...
- Fixed error when writing back to the same file (3419).
- PO msgid comments are now only parsed in msgid entries.
- DTD Newlines are now skipped when parsing (3390).
- JSON Unit ordering is maintained (3394).
- `translate.misc.multistring.multistring`:
  - Fixed list coercion to text
  - Fixed comparison regression with multistrings (3404).
  - Re-added `str` method (3428).
  - Fixed `__hash__` (3434).
