...
- Gettext PO:
  ...
  - Optimization to increase improve performance
...
- XLIFF: Correctly parse XLIFF 1.2
- Properties
...
- New conversion options:
  - --timestamp -- skip conversion if the output file has a newer timestamp
    (Makefile-alike)
  - --threshold -- in po2* converters this allows you to specify a percentage
    complete threshold.  If the PO files passes this theshold then the file is
    output (bug 2998)
  - --removeuntranslated -- Extend this option to po2dtd and thus po2moz --
    don't output untranslated text (bug 1718)
...
- Docs lots of cleanups: TBX, PHP, Added Android and JSON docs
- Requirements files document all requirements for the Translate Toolkit and
