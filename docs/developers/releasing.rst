...
tagged release to PyPI and GitHub. The GitHub release notes are generated by
converting :file:`docs/releases/$version.rst` to Markdown, so make sure the
release notes exist before pushing the tag.
...
The GitHub release job fails if :file:`docs/releases/$version.rst` is missing.
