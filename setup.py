...
infofiles = [(join(sitepackages, 'translate'),
             [join('translate', filename) for filename in 'ChangeLog', 'LICENSE', 'README.rst'])]
...
def buildinfolinks():
    ...
    for infofile in ["README.rst", "LICENSE", "ChangeLog"]:
    	if os.path.exists(infofile) or os.path.islink(infofile):
            os.remove(infofile)
    os.chdir(basedir)

    #link from COPYING to translate/LICENSE
    linkfile(os.path.join(basedir,"COPYING"), os.path.join("translate","LICENSE"))
    #make more links
    for infofile in ["README.rst", "ChangeLog"]:
	linkfile(os.path.join(basedir, infofile) , os.path.join("translate", infofile) )
...
def buildmanifest_in(file, scripts):
    ...
    for infofile in ("README.rst", "TODO", "ChangeLog", "LICENSE", "*.txt"):
