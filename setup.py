...
try:
    from sphinx.setup_command import BuildDoc  # pylint: disable=import-error
...
initfiles = [(join(sitepackages, 'translate'),
              [join('translate', '__init__.py')])]
...
try:
    ...
    class InnoScript(object):
        ...
        def __init__(self, name, lib_dir, dist_dir, exe_files=None,
                     other_files=None, install_scripts=None, version="1.0"):
            if not exe_files:
                exe_files = []
            if not other_files:
                other_files = []
        ...
        def create(self, pathname=None):
            ...
            # See http://www.jrsoftware.org/isfaq.php for more InnoSetup config
            # options.
            ofi = self.file = open(self.pathname, "w")
            ofi.write("; WARNING: This script has been created by py2exe. "
            ...
            ofi.write("\n")
            ofi.write("[Icons]\n")
            ofi.write('Name: "{group}\\Documentation"; '
                      'Filename: "{app}\\docs\\index.html";\n')
            ofi.write('Name: "{group}\\Translate Toolkit Command Prompt"; '
                      'Filename: "cmd.exe"\n')
            ofi.write('Name: "{group}\\Uninstall %s"; '
            ...
            ofi.write('Root: HKCU; Subkey: "Environment"; '
                      'ValueType: expandsz; ValueName: "Path"; '
                      'ValueData: "{reg:HKCU\\Environment,Path|};{app};"\n')
            ofi.write("\n")
            if self.install_scripts:
                ofi.write("[Run]\n")
                for path in self.install_scripts:
                    ofi.write('Filename: "{app}\\%s"; WorkingDir: "{app}"; '
                              'Parameters: "-install"\n' % path)
                ofi.write("\n")
        ...
        def compile(self):
            ...
            if result:
                ...
                print(
                    "Opening iss file, use InnoSetup GUI to compile manually")
                os.startfile(self.pathname)

    class BuildExeMap(BuildCommand):
        ...
        def reinitialize_command(self, command, reinit_subcommands=0):
            if command == "install_data":
                ...
                install_data.data_files = self.remap_data_files(
                    install_data.data_files)
                return install_data
            return BuildCommand.reinitialize_command(self, command,
                                                     reinit_subcommands)
        ...
        def __init__(self, attrs):
            ...
            py2exeoptions = {
                ...
                "excludes": [
                    ...
                    # Need to do more to support spell checking on Windows:
                    "enchant",
                    # strange things unnecessarily included with some versions
                    # of pyenchant:
                    "win32ui", "_win32sysloader", "win32pipe", "py2exe",
                    "win32com", "pywin", "isapi", "_tkinter", "win32api",
                ],
            }
...
def getdatafiles():
    ...
    def listfiles(srcdir):
        return join(sitepackages, srcdir), [join(srcdir, f)
...
def buildmanifest_in(f, scripts):
    ...
    f.write("# MANIFEST.in: the below autogenerated by setup.py "
            "from translate %s\n" % translateversion)
...
def standardsetup(name, version, custompackages=None, customdatafiles=None):
    ...
    if not customdatafiles:
        customdatafiles = []
    if not custompackages:
        custompackages = []
    try:
        ...
        sys.stderr.write("warning: could not recreate MANIFEST.in, "
                         "continuing anyway. (%s)\n" % e)
...
def dosetup(name, version, packages, datafiles, scripts, ext_modules=None):
    if not ext_modules:
        ext_modules = []
    ...
    setup(name=name,
          ...
          download_url="http://sourceforge.net/projects/"
                       "translate/files/Translate Toolkit/" + version,
          platforms=["any"],
