...
from translate_toolkit.storage.idml import INLINE_ELEMENTS, NO_TRANSLATE_ELEMENTS, open_idml
from translate_toolkit.storage.xml_extract.extract import (
    IdMaker,
    ParseState,
    build_idml_store,
    make_postore_adder,
...
def convert_idml(inputfile, outputfile, template):
    ...
    for filename, translatable_file in contents.items():
        ...
        build_idml_store(
            BytesIO(translatable_file), store, parse_state, store_adder=po_store_adder
        )
